    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative z-10 w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl border border-border bg-card shadow-xl">
        ...
        <div className="px-4 py-2 bg-muted/50">
          <p className="text-xs text-muted-foreground">Current principal: <span className="font-semibold text-foreground">₹{transaction.currentPrincipal.toLocaleString('en-IN')}</span></p>
        </div>
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 p-4">
          <div>
            ...
            <input {...register('amount')} type="number" step="0.01" placeholder="2000" className={inputCls} />
            {errors.amount && <p className="mt-1 text-xs text-red-500">{errors.amount.message}</p>}
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-foreground">Date</label>
            <input {...register('date')} type="date" max={format(new Date(), 'yyyy-MM-dd')} className={inputCls} />
          ...
          <button type="submit" disabled={isSubmitting} className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2.5 text-sm font-semibold text-white hover:bg-primary/90 disabled:opacity-50 transition-colors">
